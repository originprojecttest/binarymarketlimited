...
async function fetchAndRenderCalculatedLedgerTotals(uuid, currencySymbol = "$", fallbackPlanProfit = 0) {
    ...
    try {
        ...
        const res = await fetch(`https://broker-rho.vercel.app/api/admin-update-user?uuid=${uuid}`, {
            method: "GET",
...
document.getElementById("profileForm")?.addEventListener("submit", async (e) => {
    ...
    try {
        const response = await fetch("https://broker-rho.vercel.app/api/admin-update-user", {
            method: "PUT",
            headers: {
