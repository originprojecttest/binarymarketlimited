...
async function fetchAndRenderHistoryLogs() {
    ...
    try {
        const response = await fetch(`https://broker-rho.vercel.app/api/admin-history?uuid=${contextualUserUuidString}&table=${activeModule}&page=${currentHistoryPage}&limit=${historyRowsLimitPerPage}`, {
            method: "GET",
...
async function submitRowUpdate() {
    ...
    try {
        const response = await fetch(`https://broker-rho.vercel.app/api/admin-history?id=${activeSelectedRowData.id}&table=${activeModule}`, {
            method: "PUT",
            headers: {
...
async function deleteHistoryRecord(id) {
    ...
    if (confirm.isConfirmed) {
        try {
            const response = await fetch(`https://broker-rho.vercel.app/api/admin-history?id=${id}&table=${activeModule}`, {
                method: "DELETE",
                headers: { "Authorization": `Bearer ${adminToken}` }
            });
